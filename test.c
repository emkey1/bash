...
static __thread procenv_t test_exit_buf;
static __thread int test_error_return;
#define test_exit(val) \
	do { test_error_return = val; sh_longjmp (test_exit_buf, 1); } while (0)

extern int sh_stat PARAMS((const char *, struct stat *));

static __thread int pos;		/* The offset of the current argument in ARGV. */
static __thread int argc;	/* The number of arguments present in ARGV. */
static __thread char **argv;	/* The argument list. */
static __thread int noeval;
