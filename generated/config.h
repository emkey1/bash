 ...
 * tree is also compiled for Linux (the CI builds, and anyone building the CLI
 * there). Darwin's struct stat spells the sub-second fields st_atimespec;
 * glibc and musl spell them st_atim, and stat-time.h picks between them from
 * exactly these three macros -- so the Darwin answer expands to a member that
 * does not exist and the build stops at
 *     stat-time.h:56: 'const struct stat' has no member named 'st_atimespec'
 * Decide it by platform instead of by whichever machine last ran configure. */
#if defined(__linux__)
