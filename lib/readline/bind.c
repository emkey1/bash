...
static __thread struct {
  ...
  { "bind-tty-special-chars",	0,		0 },
  { "blink-matching-paren",	0,	V_SPECIAL },
  { "byte-oriented",		0,		0 },
#if defined (COLOR_SUPPORT)
  { "colored-completion-prefix",0,	0 },
  { "colored-stats",		0,		0 },
#endif
  { "completion-ignore-case",	0,	0 },
  { "completion-map-case",	0,	0 },
  { "convert-meta",		0, 0 },
  { "disable-completion",	0,		0 },
  { "echo-control-characters",	0,	0 },
  { "enable-active-region",	0,	0 },
  { "enable-bracketed-paste",	0,	V_SPECIAL },
  { "enable-keypad",		0,		0 },
  { "enable-meta-key",		0,		0 },
  { "expand-tilde",		0, 0 },
  { "history-preserve-point",	0,	0 },
  { "horizontal-scroll-mode",	0,	0 },
  { "input-meta",		0,			0 },
  { "mark-directories",		0,	0 },
  { "mark-modified-lines",	0,	0 },
  { "mark-symlinked-directories", 0, 0 },
  { "match-hidden-files",	0,	0 },
  { "menu-complete-display-prefix", 0, 0 },
  { "meta-flag",		0,			0 },
  { "output-meta",		0,		0 },
  { "page-completions",		0,		0 },
  { "prefer-visible-bell",	0,	V_SPECIAL },
  { "print-completions-horizontally", 0, 0 },
  { "revert-all-at-newline",	0,	0 },
  { "show-all-if-ambiguous",	0,		0 },
  { "show-all-if-unmodified",	0,	0 },
  { "show-mode-in-prompt",	0,	0 },
  { "skip-completed-text",	0,	0 },
...
/* iSH-AOK: boolean_varlist[] pointed at variables that are thread-local now,
...
aok_fix_boolean_varlist ()
{
  ...
  boolean_varlist[aok_i].value = &_rl_bind_stty_chars;
  aok_i++;
  boolean_varlist[aok_i].value = &rl_blink_matching_paren;
  aok_i++;
  boolean_varlist[aok_i].value = &rl_byte_oriented;
  aok_i++;
#if defined (COLOR_SUPPORT)
  boolean_varlist[aok_i].value = &_rl_colored_completion_prefix;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_colored_stats;
  aok_i++;
#endif
  boolean_varlist[aok_i].value = &_rl_completion_case_fold;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_completion_case_map;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_convert_meta_chars_to_ascii;
  aok_i++;
  boolean_varlist[aok_i].value = &rl_inhibit_completion;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_echo_control_chars;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_enable_active_region;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_enable_bracketed_paste;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_enable_keypad;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_enable_meta;
  aok_i++;
  boolean_varlist[aok_i].value = &rl_complete_with_tilde_expansion;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_history_preserve_point;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_horizontal_scroll_mode;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_meta_flag;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_complete_mark_directories;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_mark_modified_lines;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_complete_mark_symlink_dirs;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_match_hidden_files;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_menu_complete_prefix_first;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_meta_flag;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_output_meta_chars;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_page_completions;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_prefer_visible_bell;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_print_completions_horizontally;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_revert_all_at_newline;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_complete_show_all;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_complete_show_unmodified;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_show_mode_in_prompt;
  aok_i++;
  boolean_varlist[aok_i].value = &_rl_skip_completed_text;
  aok_i++;
#if defined (VISIBLE_STATS)
  boolean_varlist[aok_i].value = &rl_visible_stats;
  aok_i++;
#endif /* VISIBLE_STATS */
  aok_i++;
}
